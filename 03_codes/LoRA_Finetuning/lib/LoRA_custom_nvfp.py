...
from .custom_nvfp import CustomNVFPLinear

class LoRA_NVFP(torch.nn.Module, LoraLayer):
...
def dispatch_nvfp(target: torch.nn.Module, adapter_name: str, **kwargs) -> Optional[torch.nn.Module]:
    """
    Dispatcher function for CustomNVFPLinear layers.
    When a CustomNVFPLinear layer is detected, returns LoRA_NVFP wrapper.
    """
    ...
    # Check if the base layer is CustomNVFPLinear
    if isinstance(target_base_layer, CustomNVFPLinear):
        nvfp_kwargs = kwargs.copy()
        ...
        if hasattr(target_base_layer, 'block_size'):
            ...
            pass
        new_module = LoRA_NVFP(target, adapter_name, **nvfp_kwargs)
