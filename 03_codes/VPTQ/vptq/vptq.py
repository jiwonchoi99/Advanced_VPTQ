...
class VPTQ:
    ...
    def __init__(
        ...
        collect_act=False,
        layer_name='',
        block_size=128, #must be set
        percdamp=.01,
        group_size=-1,
        group_num=4, #must be set
        enable_perm=False,
        enable_norm=False,
        norm_dim=1,
        debug=False
    ...
    def fast_vector_quant(self, init_centroids=True):
        ...
        #HJ(1.29)padding
        #pdb.set_trace()
        #self.reshaper = reshape(vector_len=self.vector_lens[1], outlier_vector_len = self.vector_lens[0], enable_transpose=False, group_num = self.group_num)
        #weight, self.is_padded, self.pad_cols, self.outlier_size = self.reshaper.add_whole_padding(weight)
        #pdb.set_trace()
        ...
        #self.logger.info(f"kmeans_weight.shape = {kmeans_weight.shape}")

        # set group num and size
        self.outlier_size, self.group_size, self.group_num = \
            self.quantizer.get_group_setting(weight)
        #pdb.set_trace()
        #self.logger.info(f'group_size: {self.group_size}, group_num: {self.group_num}')
        ...
        # first round vptq (여기에서 에러 프로파게이션 진행)
    ...
    def vptq(self, weight, hessian, enable_residual=False, inv_hessian=None):
        ...
        self.outlier_size, self.group_size, self.group_num = self.quantizer.get_group_setting(weight)
        ...
        while i < self.columns:
            if self.quantizer.enable_transpose:
                self.block_size = self.block_size
            else:
                # 1. 현재 블록 크기 결정 (처음엔 outlier_size, 그 다음부터는 group_size)
                if i == 0:
                    if self.vector_lens[0] == -1:
                        self.block_size = self.group_size
                        self.step = self.vector_lens[1]
                    else:
                        self.block_size = self.outlier_size
                        self.step = self.vector_lens[0]
                else:
                    self.block_size = self.group_size
                    self.step = self.vector_lens[1]
            ...
            #self.logger.info(f'Q1.shape:{Q1.shape}') #HJ(1.23) 키면 오류남!
            ...
            for k in range(0, size, self.step):
                ...
                #self.logger.info(f'vptq(366) tile_weight:{tile_weight.shape}, tile_inv_hessian:{tile_inv_hessian.shape}')
                if enable_residual:
                    tile_qweight = self.quantizer.quantize_residual_vector(tile_weight, i + k)
                else:
                    #여기에서 선택된 tile 의 인덱스가 업데이트!
                    tile_qweight = self.quantizer.quantize_vector(tile_weight, i + k)
                #self.logger.info(f'vptq(371) tile_qweight:{tile_qweight.shape}')
                #pdb.set_trace()
                ...
                #self.logger.info(f'vptq(377) block_qweight:{block_qweight.shape}, tile_qweight:{tile_qweight.shape}')
                #self.logger.info(f'vptq(378) block_qweight[:, k:k+self.step]: {block_qweight[:, k:k+self.step].shape}')
