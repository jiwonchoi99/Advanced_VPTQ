...
class VQuantLinear(nn.Module):
    ...
    def __init__(
        ...
        if self.vector_quant_dim == "in":
            self.padding = (-self.in_features) % self.vector_len
            self.group_size = group_size
            self.transpose = False
            self.num_indices = self.out_features
        else:
        ...
        if vector_lens[0] > 1 and num_centroids[0] > 0:
            ...
            if self.vector_quant_dim == "in":
                self.outlier_padding = (-self.in_features) % self.outlier_vector_len
                self.ouliter_num_indices = self.out_features
        ...
        if self.is_indice_packed is True:
            ...
            if self.vector_quant_dim == "out":
                ...
                self.indices = Parameter(
                    torch.empty((self.num_codebooks, self.num_indices, int(packed_groupsize/self.vector_len)),
                                dtype=index_dtype,
                                device=device),
                    requires_grad=False,
                )
            ...
            if self.vector_quant_dim == "out":
                ...
                self.indices = Parameter(
                    torch.empty((self.num_codebooks, self.num_indices, int(self.group_size/self.vector_len)),
                                dtype=torch.int16,
                                device=device),
                    requires_grad=False,
                )
    ...
    def init_parameters(
        ...
        #print(f'indices[0]: {indices[0].shape}') #If Outlier 기능 is off, then must not be executed. Otherwise, the code stops due to indices[0] not exist
        for cidx in keys[1:]:
            print(f'indices[{cidx}]: {indices[cidx].shape}')
            _indices.append(indices[cidx])
        _indices = torch.stack(_indices, dim=0)
        print(f'indices_after_stack[{cidx}]: {_indices.shape}')

        # HJ(1.27) _res_indices = _res_indices.reshape(self.num_codebooks, self.num_indices, self.group_size)
        
        if self.vector_quant_dim == "out":
            _indices = _indices.reshape(self.num_codebooks, self.num_indices, self.group_size)
        else:
            _indices = _indices.reshape(self.num_codebooks, self.num_indices, math.ceil(self.group_size / self.vector_len))
        print(f'indices_after_reshape[{cidx}]: {_indices.shape}')
        ...
        if self.enable_residual:
            ...
            if self.vector_quant_dim == "out":
                ...
                _res_indices = _res_indices.reshape(self.num_codebooks, self.num_indices, int(self.group_size / self.vector_len))
    ...
    def dequant(self):
        ...
        #print(f'0 indices: {indices.shape}')
        indices = indices.unsqueeze(-1).expand(-1, -1, -1, self.vector_len)

        #print(f'1 indices: {indices.shape}')
        ...
        #print(f'3 indices: {selected_centroids.shape}')
        selected_centroids = selected_centroids.view(self.num_codebooks, -1, int(self.group_size / self.vector_len), self.vector_len)

        #print(f'4 indices: {selected_centroids.shape}')
        selected_centroids = selected_centroids.permute(0, 1, 2, 3)
        #print(f'5 indices: {selected_centroids.shape}')
        #HJ(1.29) 각 그룹마다 remove padding 시키도록 만들기.

        qweight = selected_centroids.reshape(self.num_codebooks, -1, self.group_size)
        #print(f'6 indices: {qweight.shape}')
        qweight = qweight.permute(1, 0, 2)
        #print(f'7 indices: {qweight.shape}')
        qweight = qweight.reshape(-1, self.num_codebooks * self.group_size)
        #print(f'8 indices: {qweight.shape}')
        if self.enable_residual:
            ...
            selected_res_centroids = selected_res_centroids.reshape(
                self.num_codebooks, -1, int(self.group_size / self.vector_len), self.vector_len
            )

            selected_res_centroids = selected_res_centroids.permute(0, 1, 2, 3)
        ...
        # remove padding by matching the dimension of input actiation and qweight
        #pdb.set_trace()
        ...
        if self.enable_outlier:
            ...
            #print(f'1 outlier_indices: {outlier_indices.shape}')
            outlier_indices = outlier_indices.unsqueeze(-1).expand(-1, -1, -1, self.outlier_vector_len)
            #print(f'2 outlier_indices: {outlier_indices.shape}')
            outlier_indices = outlier_indices.reshape(1, -1, self.outlier_vector_len)
            #print(f'3 outlier_indices: {outlier_indices.shape}')

            selected_outlier_centroids = torch.gather(outlier_centroids, 1, outlier_indices)
            #print(f'4 selected_outlier_centroids: {selected_outlier_centroids.shape}')
            selected_outlier_centroids = selected_outlier_centroids.reshape(
                1, -1, int(self.outlier_size / self.outlier_vector_len) , self.outlier_vector_len
            )
            #print(f'5 selected_outlier_centroids: {selected_outlier_centroids.shape}')
            selected_outlier_centroids = selected_outlier_centroids.permute(0, 1, 2, 3)
            #print(f'6 selected_outlier_centroids: {selected_outlier_centroids.shape}')
            qweight_outlier = selected_outlier_centroids.reshape(-1, self.outlier_size)
            #print(f'7 qweight_outlier: {qweight_outlier.shape}')
            ...
            #print(f'8 qweight: {qweight.shape}')
        if self.enable_perm:
            if self.is_indice_packed:
    ...
    def forward(self, x):
        ...
        input_in_feats = x.shape[-1]  # 입력의 마지막 차원 (4096)
        weight_in_feats = qweight.shape[1]  # 가중치의 입력 차원 (4104)
        
        #pdb.set_trace()

        if weight_in_feats > input_in_feats:
            # [:, :input_in_feats] -> 모든 행(Row)은 유지하되, 열(Column)은 필요한 만큼만 남김
            qweight = qweight[:, :input_in_feats]
