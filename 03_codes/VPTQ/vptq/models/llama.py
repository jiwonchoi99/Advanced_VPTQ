...
def is_layer_done(layer_idx, args):
    state_path = f'{args.output_dir}/qlinear_layer_state_{layer_idx}.pt'
    args_path  = f'{args.output_dir}/qlinear_args_{layer_idx}.pt'
    return os.path.exists(state_path) and os.path.exists(args_path)
    
# YERI : Load the quantized layers
def load_all_qlinear_from_disk(args, layers):
    layer_state_dicts = {}
    layer_qlinear_args = {}
    for layer_idx in range(len(layers)):
        sd = torch.load(f'{args.output_dir}/qlinear_layer_state_{layer_idx}.pt',
                        map_location='cpu', weights_only=False)
        for key, value in sd.items():
            if "indices" in key:
                sd[key] = value.view(torch.uint16)

        qa = torch.load(f'{args.output_dir}/qlinear_args_{layer_idx}.pt',
                        map_location='cpu', weights_only=False)

        layer_state_dicts[layer_idx] = sd
        layer_qlinear_args[layer_idx] = qa
...
def quant_llama(model, args, quant_args, dev='cuda'):
    ...
    quantizers = {}
    layers = model.model.layers
    # YERI: only un-quantized layers (resume even if the execution is interrupted)
    pending_layers = []
    ...
    if len(pending_layers) == 0 or (args.eval_mode == True):
        ...
        #HJ (1.26) evaluation 할 때 Indices Not Found 라는 에러가 떠서 이 한 줄 추가해줌.
        model = qmodel
    ...
    print(f'\n\n----quantization start ...---- {time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())}')
    ...
    for gpu_idx in range(num_gpus):
        ...
        for _ in range(layers_for_this_gpu):
            ...
            current_gpu_tasks.append(pending_layers[current_layer_idx])
            current_layer_idx += 1
    ...
    env_devices = os.environ.get("CUDA_VISIBLE_DEVICES", None)
    env_devices = env_devices.split(',')

    if args.num_gpus == 1:
        layer_state_dicts, layer_qlinear_args = quantize_executer(0, tasks[0], args, quant_args, None, None)
    else:
        for gpu_idx in range(num_gpus):
            gpu_idx_arg = env_devices[gpu_idx]
            # we have to set CUDA_VISIBLE_DEVICES here
            # cuml only supports to run on GPU:0
            print(f"gpu {gpu_idx_arg}")
            os.environ["CUDA_VISIBLE_DEVICES"] = str(gpu_idx_arg)
    ...
    print(f'\n\n----quantization done ...---- {time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())}')
    ...
    if len(layer_state_dicts) != len(layers):
        ...
        print("Success: all layers are quantized")
...
def get_quantized_llama(model_name, seqlen, layer_state_dicts, layer_qlinear_args):
    ...
    for layer_idx, layer_state_dict in layer_state_dicts.items():
        ...
        for name, opin in ops.items():
            if layer_idx not in layer_qlinear_args:
                continue
            if name not in layer_qlinear_args[layer_idx]:
                continue
        ...
        # YERI: layer state reshape because direction changed to ich
        fixed_state = {}
        ...
        for k, v in layer_state_dict.items():
            if v.dtype not in {
                ...
                torch.uint64, torch.uint32, torch.uint16, torch.uint8,
                torch.bool,
            }:
                fixed_state[k] = v.to(dtype)
            else:
                fixed_state[k] = v
        ...
        for k, v in fixed_state.items():
            if 'indices' in k:
                sub_mod_name = k.rsplit('.', 1)[0]
                param_name = k.rsplit('.', 1)[1]
                
                target_mod = layer
                for attr in sub_mod_name.split('.'):
                    target_mod = getattr(target_mod, attr)
                
                new_param = torch.nn.Parameter(torch.empty_like(v), requires_grad=False)
                setattr(target_mod, param_name, new_param)

        layer.load_state_dict(fixed_state, strict=False)
