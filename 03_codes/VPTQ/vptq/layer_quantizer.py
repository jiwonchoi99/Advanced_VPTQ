...
def layer_quantizer(args, quant_args, layer, layer_idx, logger, dev, dtype, name2hessian=None):
    ...
    print(opeartor_names)
    # with torch.no_grad():
    for names in opeartor_names:
    #for names in [['mlp.down_proj', 'self_attn.v_proj', 'self_attn.q_proj', 'self_attn.k_proj', 'self_attn.o_proj', 'mlp.up_proj','mlp.gate_proj']]:
        ...
        for name in subset:
            ...
            hessian, mu = load_hessian(quant_args, hessian_path, logger)

            #add padding to hessian & inv_hessian
            ...
            if args.inv_hessian_path is not None:
                ...
                #add padding to hessian & inv_hessian
                inv_hessian, perm, zero_idx = load_inv_hessian(quant_args, inv_hessian_path, logger)
                inv_hessian.to('cpu')
                perm.to('cpu')
                zero_idx.to('cpu')
            else:
            ...
            quantizer = NPVectorQuantizer(
                ...
                enable_transpose=quant_args.enable_transpose,
                kmeans_mode=quant_args.kmeans_mode,
                iter=quant_args.kiter,
                tol=quant_args.ktol,
                enable_norm=quant_args.enable_norm,
                norm_dim=quant_args.norm_dim,
                debug=True,
                bitwidth = quant_args.bitwidth,
                bsize = quant_args.bsize,
            )
            ...
            #padding size
            #padding_size = in_features - hessian.shape[1]
            ...
            qlayer = VQuantLinear(
                ...
                norm_dim=quant_args.norm_dim,
                enable_perm=quant_args.enable_perm,
                vector_quant_dim=quant_args.vector_quant_dim,
                device=dev,
                dtype=dtype,
