...
class NPVectorQuantizer:
    ...
    """
    def quantize_centroids(centroids):
            import os
            ...
            print(f"Codebook shape: {list(centroids.shape)}, bsize = {self.bsize}")

            qcentroids = centroids.clone()

            imin, imax = -(2 ** (self.bitwidth - 1)), 2 ** (self.bitwidth - 1) - 1
            num_codebook, code_size = centroids.shape

            qmaxes = []

            for start in range(0, num_codebook, self.bsize):
                end = min(start + self.bsize, num_codebook)
            ...
            if first_call:
                np.save(os.path.join(save_dir, "centroids_after.npy"),
                        centroids.detach().cpu().numpy())
                np.save(os.path.join(save_dir, f"qmax{self.bsize}.npy"),
    ...
    def init_centroids_indices(self, data, weights=None):
        ...
        for idx, (i, j) in enumerate(self.get_index_list(data)):
            ...
            if num_centroids == -1:  # Do not quantize, keep original data
                ...
                if self.bitwidth < 16:
                    self.logger.info(f"quantized_centroids bsize = {self.bsize}\n")
                    _centroids = quantize_centroids(
                            _centroids,
